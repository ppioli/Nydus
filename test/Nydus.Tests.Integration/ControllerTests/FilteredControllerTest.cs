...
public class FilteredControllerTest : ControllerTestBase
{
    public FilteredControllerTest() : base("Filtered")
    {
    }
    ...
    [Fact]
    public async Task FilterByGenderMale()
    {
        var actualResponse = await GetResponse<List<User>>(
            new[]
            {
                new KeyValuePair<string, string>("isMale", "true"),
            });

        Assert.All(actualResponse, user => Assert.Equal("M", user.Gender));
    }

    [Fact]
    public async Task FilterByGenderFemale()
    {
        var actualResponse = await GetResponse<List<User>>(
            new[]
            {
                new KeyValuePair<string, string>("isMale", "false"),
            });

        Assert.All(actualResponse, user => Assert.Equal("F", user.Gender));
    }

    [Fact]
    public async Task FilterByGenderFemaleParam()
    {
        var actualResponse = await GetResponse<List<User>>(
            "Filtered",
            new[]
    ...
    // {
    //     var actualResponse = await GetResponse<List<User>>(new[]
    //     {
    ...
    //     for (var i = 1; i < actualResponse.Count; i++)
    //     {
    //         var previousValueIsSmaller = String.Compare(actualResponse[i].UserName, actualResponse[i - 1].UserName,
    ...
    //     for (var i = 1; i < actualResponse.Count; i++)
    //     {
    //         var previousValueIsSmaller = String.Compare(actualResponse[i].Email, actualResponse[i - 1].Email,
    //             StringComparison.Ordinal) > 0;
    //         Assert.True(previousValueIsSmaller);
    //     }
    // }
    ...
    //     {
    //         var previousValueIsSmaller =
    //             string.Compare(actualResponse[i].FavoriteAnimal.Name, actualResponse[i - 1].FavoriteAnimal.Name,
    //                 StringComparison.Ordinal) > 0;
    //         Assert.True(previousValueIsSmaller);
    //     }
    // }
